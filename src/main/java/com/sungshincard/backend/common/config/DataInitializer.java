...
import com.sungshincard.backend.domain.product.repository.EvolutionStageRepository;
...
public class DataInitializer implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    if (evolutionStageRepository.count() == 0) {
      initEvolutionStages();
    }
  }
  ...
  private void initCategories() {
    ...
    cardCategoryRepository.save(CardCategory.builder()
        .name("BASIC_ENERGY").displayName("기본 에너지").parent(energy).gameType(CardMaster.GameType.POKEMON).sortOrder(1)
        .build());
    cardCategoryRepository.save(CardCategory.builder()
        .name("SPECIAL_ENERGY").displayName("특수 에너지").parent(energy).gameType(CardMaster.GameType.POKEMON).sortOrder(2)
        .build());
    ...
    cardCategoryRepository.save(CardCategory.builder()
        .name("STADIUM").displayName("스타디움").parent(trainer).gameType(CardMaster.GameType.POKEMON).sortOrder(3)
        .build());
    cardCategoryRepository.save(CardCategory.builder()
        .name("TOOL").displayName("포켓몬 도구").parent(trainer).gameType(CardMaster.GameType.POKEMON).sortOrder(4).build());
  }

  private void initEvolutionStages() {
    String[] stages = { "기본", "1진화", "2진화", "V", "VMAX", "VSTAR", "ex", "메가 ex", "라디언트" };
    for (int i = 0; i < stages.length; i++) {
        evolutionStageRepository.save(EvolutionStage.builder()
          .name(stages[i])
